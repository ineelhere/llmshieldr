...
`shield_gemini()` gives the same guarded workflow through the Gemini Developer
API. Set `GEMINI_API_KEY` or `GOOGLE_API_KEY` in your environment. The assistant
defaults to `gemini-2.5-flash` and the separate reviewer to
`gemini-2.5-flash-lite`; both had free-tier access when documented. Check
