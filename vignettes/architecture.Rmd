...
context_policy() defines provenance and authorization requirements
scan_context() checks and admits retrieved rows before prompt assembly
scan_conversation() checks role-preserving chat histories
tool_policy() and guard_tool() authorize, scan, dispatch, and rescan tools
stream_guard() buffers provider chunks and releases only final safe text
scan_document() performs bounded extraction before content is trusted
scan_output() checks model text before display, storage, or downstream use
output_contract() and grounding_policy() validate destination and citations
secure_chat() orchestrates provider creation, checks, execution, and audit
telemetry_options() exports content-free decision events
write_audit_log() persists the evidence trail with explicit content controls
```
...
- Keep internals inspectable: policies are lists of explicit rules, not a
  hidden classifier.
- Support deterministic rules, local NLP checks, and optional semantic review
  with Gemini, Ollama, other ellmer providers, or caller-supplied reviewers.
- Stay model-agnostic: any provider exposed through `ellmer::chat()`, existing
  chat object, object with `$chat()`, or plain R function can be used.
...
1. **Decision model:** explicit rules, findings, reports, policy thresholds,
   orchestration controls, and OWASP 2026 metadata.
2. **Detection:** normalization, deterministic rules, local NLP intent,
   optional semantic reviewers, entity recognizers, and secret detectors.
3. **Admission:** context provenance and authorization, bounded document
   extraction, URL policy, provider trust boundaries, and rate budgets.
4. **Execution:** provider-neutral chat orchestration and default-deny tool
   policies with schema, authorization, spend, and loop controls.
5. **Release:** output scanning, grounding checks, destination contracts, and
   buffered stream release.
6. **Evidence:** metadata-first audits, privacy-safe telemetry, execution
   statistics, evaluation summaries, and policy comparisons.
...
- Use `context_policy()`, `tool_policy()`, `output_contract()`, and
  `grounding_policy()` to make workflow boundaries explicit.
- Add external detectors with `guardrail_provider()`, PII services with
  `presidio_provider()`, and custom secret patterns with `secret_registry()`.
...
For future CRAN releases, regenerate documentation, run the test suite, run
