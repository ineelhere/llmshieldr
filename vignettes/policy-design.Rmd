...
Malformed JSON and schema issues are recorded failures. The scanner warns, keeps
deterministic findings, and stores structured reviewer errors in
`report$metadata$reviewer_errors`. The default policy adds a blocking reviewer
failure finding; configure `on_reviewer_error` explicitly to escalate or use a
rules-only fallback.
:::
...
LLM09:2026 findings. If the policy has `trusted_sources`, rows whose source is
missing or outside that allowlist receive a critical OWASP LLM01:2026 finding
and an admission decision of `drop`. Use `authorize` for row-level permission
checks.
...
findings; an explicit critical block action still blocks the row.
...
llm03 = excessive agency and unsafe tool-use findings
llm04 = model and provider supply-chain findings
llm05 = context provenance and poisoning findings
llm06 = resource exhaustion and rate-guard findings
llm07 = misinformation and grounding findings
llm08 = hidden-context exposure findings
llm09 = retrieved-context and vector-isolation findings
llm10 = improper output handling findings
```
