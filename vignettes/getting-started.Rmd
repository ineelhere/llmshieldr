...
To use a real provider, replace the callback with a provider name and model.
Keep the first experiment small:

```{r provider-next-step, eval = FALSE}
secure_chat(
...
For Ollama, change `provider` to `"ollama"` and `model` to the local model
name. The dedicated provider vignette contains one runnable example for each
provider and the required `.Renviron` setup. No provider call is made from this
