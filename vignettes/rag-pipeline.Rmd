...
rows, calls the provider, scans the raw output, and returns a `shieldr_result`.
...
assembled prompt because blocked rows are removed before the provider call.
...
Audit timing is stored as `elapsed_ms`. Token usage is estimated as
`ceiling(nchar(prompt_and_output) / 4)`, so it is useful for rate guards and
trend monitoring but not a billing-grade tokenizer.
