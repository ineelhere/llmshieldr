...
cases <- read.csv(path, stringsAsFactors = FALSE, encoding = "UTF-8")
...
Summarize sensitivity, benign false-positive rate, action accuracy, Wilson
95% confidence intervals, and p50/p95 latency:

```{r}
summarize_security_evaluation(results)
```

Before changing a production policy, compare decisions and triggered rules on
the same corpus:

```{r, eval = FALSE}
changes <- compare_policies(
  cases,
  from = policy("enterprise_default"),
  to = policy("comprehensive")
)
subset(changes, changed)
```
